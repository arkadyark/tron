...
parameters = {'near':{'voronoi_weight':3.5, 'powerup_weight':3, 'territory_weight':0.3, 'max_powerup_dist':40},
              'far':{'voronoi_weight':5.0, 'powerup_weight':7, 'territory_weight':0.8, 'max_powerup_dist':40},
              'endgame':{'voronoi_weight':6.0, 'powerup_weight':2, 'territory_weight':0.9, 'max_powerup_dist':20}}
              
def heuristic(player, opponent, game_map):
    ...
    #if game_map[opp_pos[0]][opp_pos[1]] in (WALL, TRAIL, LIGHTCYCLE):
    #    o_fringe.pop()
    ...
    distance_to_powerup = 0 if game_map[play_pos[0]][play_pos[1]] == POWERUP else (40 if num_powerups else 0)
    
    while (len (p_fringe) > 0 or len (o_fringe) > 0) and cur_dist < 25:
    ...
    if game_map[play_pos[0]][play_pos[1]] == POWERUP:
        score += 70
    ...
    print "Powerup score", -(distance_to_powerup)*params['powerup_weight']
