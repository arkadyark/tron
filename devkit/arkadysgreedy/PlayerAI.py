...
class PlayerAI():
    ...
    def get_move(self, game_map, player_lightcycle, opponent_lightcycle, moveNumber):
        start = time.time()
        print "WE ARE AT",player_lightcycle['position'][0], player_lightcycle['position'][1]        
        g = greedy_search(game_map, player_lightcycle, opponent_lightcycle) [0]
        
        if player_lightcycle['hasPowerup']:
            g = PlayerAI.activate_powerups[g]
        print moveNumber, "Took", (time.time() - start), "seconds to run"
        return g
